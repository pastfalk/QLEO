...
function disp_det(omega,k,splcoeff1,splcoeff2)
     ...
     if(mode(m).eq.0) then
       ...
       !for particles with arbitrary gyrotropic velocity distributions
        ...
        do while(esc(1).or.esc(2).or.esc(3).or.esc(4).or.esc(5).or.esc(6))
           ...
           !integratw over perpendicular velocity

           !$omp parallel do private(iperp,Ivpe_dummy) firstprivate(n,m,iarb,k,esc)
           ...
           !integrate over parallel velocity and compute total value of the 2d velocity integrals required for the evaluation of the dielectric tensor
  ...
  !the dispersion relation for a collisionless plasma can generally be written as:
  !0=det(kk - 1k^2 + omega^2/c^2 * epsilon)

  !to rewrite this in the form given below, use  k_x = k_para = k*cos(theta), k_y = 0, k_z = k_perp = k*sin(theta) and use the symmetry relations of the dielectric tensor
