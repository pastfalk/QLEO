!> Analytic solution of the wavenumber integration of term T13 = dB_k^2 * omega/k /(omega-k*vpa-sgn*mu*q) over interval [k1,k2]
...
!! \param t3 result of the integration
subroutine Term3(k1,k2,k0,c_om,c_ga,c_Bk,vpa,sgn,t3)
  use param_mod
  implicit none
  real :: k1, k2
  ...
  if(vpa.eq.0.0) then
     ...
     tk1=   B(6)/3.0 * k1**3 +&
          ...
          & lambda(1)*k1+&
          & (-alpha(0)*log(k1))/(k0(1)*k0(2)*k0(3))

     tk1=tk1+0.5*&
          ( alpha(3)*k0(1)**2 + alpha(2)*k0(1) +&
          & alpha(1)+alpha(0)/k0(1))*&
     ...
     if(k1.lt.k2) then
        ...
        if((real(k0(1)).ge.k1).and.(real(k0(1)).lt.k2)) then
           ...
           call Term3_pole(k0(1),k0(2),k0(3),alpha,t3pole)
           ...
           contr_pole=contr_pole+i*pi*sigma*t3pole

        endif

        if((real(k0(2)).ge.k1).and.(real(k0(2)).lt.k2)) then

           call Term3_pole(k0(2),k0(1),k0(3),alpha,t3pole)
        ...
        if((real(k0(3)).ge.k1).and.(real(k0(3)).lt.k2)) then
           ...
           call Term3_pole(k0(3),k0(1),k0(2),alpha,t3pole)
     ...
     t3=aimag(((tk2-tk1)+contr_pole)/(c_om(3)+i*c_ga(3)))
